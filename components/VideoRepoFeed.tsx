...
import VideoRepoCard from "./VideoRepoCard";

interface VideoRepoFeedProps {
  repos: Repository[];
  videoMap?: Map<string, string>; // repo id -> video URL
  feedItems?: FeedItem[]; // Optional feed items with actor info
}

export default function VideoRepoFeed({
  repos,
  videoMap,
  feedItems,
}: VideoRepoFeedProps) {
        ...
        return (
          ...
          >
            <VideoRepoCard
              ...
              videoUrl={videoMap?.get(repo.id.toString())}
              feedItem={feedItem}
            />
          </div>
        );
